...
In addition to the common JSCalendar object properties <<jscalendar-props>> a Event has the
...
The zero or positive duration of the event in the event's start time zone. The end time of an
...
trans-continental flight). This can be expressed using the `relativeTo` and `timeZone`
...
The scheduling status (<<sharing-scheduling-properties>>) of a Event. If set, it MUST be one
...
Represents the percent completion of the task overall. The property value MUST be a positive
...
The source from which updated versions of this group may be retrieved from. The value MUST be
a URI.
