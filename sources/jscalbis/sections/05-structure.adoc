...
A JSCalendar object is a JSON object <<RFC8259>>, which *MUST* be valid I-JSON
(a stricter subset of JSON) <<RFC7493>>. Property names and values are case
sensitive.
...
exception to this rule is the <<type-PatchObject,PatchObject>>.
...
JSCalendar objects and types. This is because the notion of equivalence might
range from byte-level equivalence to semantic equivalence, depending on the
respective use case. Normalization of JSCalendar objects is hindered because of
...
*MUST* include an `@type` property, or it *MUST* explicitly be specified to not
